...
<section class="iq-main-slider p-0">
   <div id="tvshows-slider">
      ...
      @php
         $sliders=App\Models\admin\Slider::where('type','2')->get();
      @endphp
      @if (count($sliders)>0)
         {{-- expr --}}
      ...
      <div class="dropdown-menu three-column" aria-labelledby="dropdownMenuButton">
         ...
         @if (!is_null($languages))
            @foreach ($languages as $language)
               ...
               <a class="dropdown-item" href="{{ route('all.movie',$language->id) }}/lang">{{$language->language_name}}</a>
             @endforeach
            @endif
           ...
           @if (!is_null($categories))
            @foreach ($categories as $category)
              <a class="dropdown-item" href="{{ route('all.movie',$category->id) }}/cat">{{$category->category_name}}</a>
            @endforeach
          @endif
      ...
      <div class="row">
         <div class="col-sm-12 overflow-hidden">
            <div class="iq-main-header d-flex align-items-center justify-content-between">
               <h4 class="main-title">Upcomming Movie and TV Show List</h4>
            </div>
            <div class="favorites-contens">
               <ul class="favorites-slider list-inline  row p-0 mb-0">
                  ...
                  <li class="slide-item">
                     <a href="{{ route('movie.show.detail',$item->slug) }}">
                        <div class="block-images position-relative">
                           <div class="img-box">
                              <img src="{{asset('assets/backend/video/store/thumbnail/small/'.$item->thumbnail)}}" class="img-fluid" alt="">
                           </div>
                           <div class="block-description">
                              <h6>{{$item->name}}</h6>
                              <div class="movie-time d-flex align-items-center my-2">
                                 <div class="badge badge-secondary p-1 mr-2">{{$item->view}}</div>
